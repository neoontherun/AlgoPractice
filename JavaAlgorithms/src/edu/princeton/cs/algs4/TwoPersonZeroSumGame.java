...
/**
 * The {@code TwoPersonZeroSumGame} class represents a data type for computing
 * optimal row and column strategies to two-person zero-sum games.
 * <p>
 * This implementation solves an <em>m</em>-by-<em>n</em> two-person zero-sum
 * game by reducing it to a linear programming problem. Assuming the payoff
 * matrix <em>A</em> is strictly positive, the optimal row and column player
 * strategies x* and y* are obtained by solving the following primal and dual
 * pair of linear programs, scaling the results to be probability distributions.
 * <blockquote>
 * 
 * <pre>
 *  (P)  max  y^T 1           (D)  min   1^T x
 ...
 * </blockquote>
 * <p>
 * If the payoff matrix <em>A</em> has any negative entries, we add the same
 * constant to every entry so that every entry is positive. This increases the
 * value of the game by that constant, but does not change solutions to the
 * two-person zero-sum game.
 * <p>
 * This implementation is not suitable for large inputs, as it calls a
 * bare-bones linear programming solver that is neither fast nor robust with
 * respect to floating-point roundoff error.
 * <p>
...
public class TwoPersonZeroSumGame {
	...
	/**
	 * Determines an optimal solution to the two-sum zero-sum game with the
	 ...
	 *
	 * @return the optimal row strategy <em>x</em> of this two-person zero-sum
	 *         game
	 */
	public double[] row() {
		double scale = scale();
	 ...
	 *
	 * @return the optimal column strategy <em>y</em> of this two-person
	 *         zero-sum game
	 */
	...
	private boolean isNashEquilibrium(double[][] payoff) {
		...
		// given row player's mixed strategy, find column player's best pure
		// strategy
		...
		// given column player's mixed strategy, find row player's best pure
		// strategy
