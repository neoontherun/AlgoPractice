 ...
 * iterating over all of the keys. A symbol table implements the <em>associative
 ...
 * <p>
 * This unordered symbol table class implements PATRICIA (Practical Algorithm to
 ...
 * value, except for the string of zero length (the empty string).
 * <p>
 * Unlike other generic symbol table implementations that can accept a
 * parameterized key type, this symbol table class can only accommodate keys of
 * class {@link java.lang.String}. This unfortunate restriction stems from a
 ...
 * space-optimized trie. In a classical trie, each level represents a subsequent
 * digit in a key. In PATRICIA, nodes only exist to identify the digits (bits)
 * that distinguish the individual keys within the trie. Because PATRICIA uses a
 * radix of two, each node has only two children, like a binary tree. Also like
 * a binary tree, the number of nodes, within the trie, equals the number of
 * keys. Consequently, some classify PATRICIA as a tree.
 * <p>
 * The analysis of PATRICIA is complicated. The theoretical wost-case
 * performance for a <em>get</em>, <em>put</em>, or <em>delete</em> operation is
 * <strong>O(N)</strong>, when <strong>N</strong> is less than
 ...
 * typical use. The average (and usual) performance of PATRICIA is approximately
 * <strong>~lg N</strong> for each <em>get</em>, <em>put</em>, or
 ...
 * single-bit test operations (under PATRICIA), and not full-key comparisons (as
 * required by binary trees). After the single-bit tests conclude, PATRICIA
 ...
 * <p>
 * In practice, decent implementations of PATRICIA can often outperform balanced
 * binary trees, and even hash tables. Although this particular implementation
 * performs well, the source code was written with an emphasis on clarity, and
 * not performance. PATRICIA performs admirably when its bit-testing loops are
 * well tuned. Consider using the source code as a guide, should you need to
 * produce an optimized implementation, for anther key type, or in another
...
public class PatriciaST<Value> {
	 ...
	 * contains the specified key, then its associated value becomes updated. If
	 * the value provided is {@code null}, then the key becomes removed from the
	 * symbol table.
	 * 
	 * @param key
	 *            the key
	...
	/**
	 * Returns {@code true} if the key-value pair, specified by the given key,
	 * exists within the symbol table.
	 * 
	 * @param key
	 *            the key
	 * @return {@code true} if this symbol table contains the given {@code key}
	 ...
	 *
	 * The inner loops of the get and put methods flow much better when they are
	 * not concerned with the lengths of strings, so a trick is employed to
	 ...
	 * and it is necessary. Simply padding with '\u0000' is insufficient to make
	 * all unique Unicode strings "look" unique to the get and put methods
	...
	/*
	 * Like the safeBitTest function, the safeCharAt function makes every string
	 * look like an "infinite" sequence of characters. Logically, every string
	 * gets a '\uffff' character, followed by an "infinite" sequence of '\u0000'
	 ...
	 * two characters that do not match). Also, the least significant bits of an
	 * individual character are examined first. There are many Unicode alphabets
	 * where most (if not all) of the "action" occurs in the least significant
	 * bits.
	 *
	...
	/**
	 * Unit tests the {@code PatriciaST} data type. This test fixture runs a
	 * series of tests on a randomly generated dataset. You may specify up to
	 * two integer parameters on the command line. The first parameter indicates
	 * the size of the dataset. The second parameter controls the number of
	 * passes (a new random dataset becomes generated at the start of each
	 * pass).
	 *
	 * @param args
