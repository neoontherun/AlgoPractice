...
/**
 * The {@code DirectedCycleX} class represents a data type for determining
 ...
 * <p>
 * This implementation uses a nonrecursive, queue-based algorithm. The
 ...
 * number of edges. Afterwards, the <em>hasCycle</em> operation takes constant
 * time; the <em>cycle</em> operation takes time proportional to the length of
 * the cycle.
 * <p>
 * See {@link DirectedCycle} for a recursive version that uses depth-first
 * search. See {@link Topological} or {@link TopologicalX} to compute a
 * topological order when the digraph is acyclic.
 * <p>
...
public class DirectedCycleX {
	private Stack<Integer> cycle; // the directed cycle; null if digraph is
									// acyclic
