...
/**
 * The {@code DirectedEulerianCycle} class represents a data type for finding an
 * Eulerian cycle or path in a digraph. An <em>Eulerian cycle</em> is a cycle
 * (not necessarily simple) that uses every edge in the digraph exactly once.
 * <p>
 * This implementation uses a nonrecursive depth-first search. The constructor
 * runs in O(<Em>E</em> + <em>V</em>) time, and uses O(<em>V</em>) extra space,
 * where <em>E</em> is the number of edges and <em>V</em> the number of vertices
 * All other methods take O(1) time.
 * <p>
 * To compute Eulerian paths in digraphs, see {@link DirectedEulerianPath}. To
...
public class DirectedEulerianCycle {
	private Stack<Integer> cycle = null; // Eulerian cycle; null if no such
											// cylce
