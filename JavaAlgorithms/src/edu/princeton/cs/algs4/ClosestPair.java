...
/**
 * The {@code ClosestPair} data type computes a closest pair of points in a set
 * of <em>n</em> points in the plane and provides accessor methods for getting
 * the closest pair of points and the distance between them. The distance
 * between two points is their Euclidean distance.
 * <p>
 * This implementation uses a divide-and-conquer algorithm. It runs in
...
public class ClosestPair {
	 ...
	 *             if {@code points} is {@code null} or if any entry in
			...
			for (int j = i + 1; (j < m) && (aux[j].y() - aux[i].y() < delta); j++) {
				...
				if (distance < delta) {
					...
					if (distance < bestDistance) {
						...
						best1 = aux[i];
						best2 = aux[j];
						// StdOut.println("better distance = " + delta + " from
						// " + best1 + " to " + best2);
					}
				}
			}
	 ...
	 *
	 * @return one of the two points in the closest pair of points; {@code null}
	 ...
	 *
	 * @return the other point in the closest pair of points {@code null} if no
	 ...
	 *         {@code Double.POSITIVE_INFINITY} if no such pair of points exist
	...
	/**
	 * Unit tests the {@code ClosestPair} data type. Reads in an integer
	 * {@code n} and {@code n} points (specified by their <em>x</em>- and
	 * <em>y</em>-coordinates) from standard input; computes a closest pair of
	 * points; and prints the pair to standard output.
	 *
	 * @param args
