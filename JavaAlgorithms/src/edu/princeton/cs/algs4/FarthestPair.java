 ...
 * and uses O(<em>N</em>) extra space. See also {@link ClosestPair} and
 * {@link GrahamScan}.
 * <p>
...
public class FarthestPair {
	...
	public FarthestPair(Point2D[] points) {
		...
		for (int i = 1; i <= k && j <= m; i++) {
			...
			while ((j < m) && Point2D.area2(hull[i], hull[i + 1], hull[j + 1]) > Point2D.area2(hull[i], hull[i + 1], hull[j])) {
				j++;
				// StdOut.println(hull[i] + " and " + hull[j] + " are
				// antipodal");
	 ...
	 *         {@code null} if no such point (because there are fewer than 2
	 *         points)
	 */
	...
	/**
	 * Returns the Eucliden distance between the farthest pair of points. This
	 * quantity is also known as the <em>diameter</em> of the set of points.
	 *
	...
	/**
	 * Unit tests the {@code FarthestPair} data type. Reads in an integer
	 * {@code n} and {@code n} points (specified by their <em>x</em>- and
	 * <em>y</em>-coordinates) from standard input; computes a farthest pair of
