...
/**
 * The {@code DirectedEulerianPath} class represents a data type for finding an
 * Eulerian path in a digraph. An <em>Eulerian path</em> is a path (not
 * necessarily simple) that uses every edge in the digraph exactly once.
 * <p>
 * This implementation uses a nonrecursive depth-first search. The constructor
 * runs in O(E + V) time, and uses O(V) extra space, where E is the number of
 * edges and V the number of vertices All other methods take O(1) time.
 * <p>
 * To compute Eulerian cycles in digraphs, see {@link DirectedEulerianCycle}. To
 * compute Eulerian cycles and paths in undirected graphs, see
...
public class DirectedEulerianPath {
	...
	public DirectedEulerianPath(Digraph G) {
		...
		// special case for digraph with zero edges (has a degenerate Eulerian
		// path)
		if (s == -1)
			s = 0;
