...
/**
 * The {@code GaussJordanElimination} data type provides methods to solve a
 * linear system of equations <em>Ax</em> = <em>b</em>, where <em>A</em> is an
 * <em>n</em>-by-<em>n</em> matrix and <em>b</em> is a length <em>n</em> vector.
 * If no solution exists, it finds a solution <em>y</em> to <em>yA</em> = 0,
 * <em>yb</em> &ne; 0, which which serves as a certificate of infeasibility.
 * <p>
 * This implementation uses Gauss-Jordan elimination with partial pivoting. See
 * {@link GaussianElimination} for an implementation that uses Gaussian
 * elimination (but does not provide the certificate of infeasibility). For an
 * industrial-strength numerical linear algebra library, see
...
public class GaussJordanElimination {
	 ...
	 * <em>A</em> is an <em>n</em>-by-<em>n</em> matrix and <em>b</em> is a
	 * length <em>n</em> vector.
	 *
	 * @param A
	...
	public GaussJordanElimination(double[][] A, double[] b) {
		...
		// only needed if you want to find certificate of infeasibility (or
		// compute inverse)
		for (int i = 0; i < n; i++)
			a[i][n + i] = 1.0;
	...
	private void solve() {
		...
		for (int p = 0; p < n; p++) {
			...
			if (Math.abs(a[p][p]) <= EPSILON) {
				continue;
				// throw new ArithmeticException("Matrix is singular or nearly
				// singular");
			}
	 ...
	 *         <em>yA</em> = 0, <em>yb</em> &ne; 0; {@code null} if no such
	 *         solution
	 */
	public double[] dual() {
