...
public class HopcroftKarp {
	...
	public HopcroftKarp(Graph G) {
		...
		// the call to hasAugmentingPath() provides enough info to reconstruct
		// level graph
			...
			for (int s = 0; s < V; s++) {
				...
				while (!path.isEmpty()) {
					...
					// advance
						...
						if (!isMatched(w)) {
							// StdOut.println("augmenting path: " +
							// toString(path));
			...
			for (int w : G.adj(v)) {
				...
				if (isResidualGraphEdge(v, w)) {
					if (!marked[w]) {
						...
						// stop enqueuing vertices once an alternating path has
						// been discovered
						// (no vertex on same side will be marked if its
						// shortest path distance longer)
	 ...
	 * arguments {@code V1}, {@code V2}, and {@code E}; creates a random
	 * bipartite graph with {@code V1} + {@code V2} vertices and {@code E}
	 * edges; computes a maximum matching and minimum vertex cover; and prints
	 * the results.
	 *
	 * @param args
