...
/**
 * The {@code FlowNetwork} class represents a capacitated network with vertices
 * named 0 through <em>V</em> - 1, where each directed edge is of type
 * {@link FlowEdge} and has a real-valued capacity and flow. It supports the
 * following two primary operations: add an edge to the network, iterate over
 * all of the edges incident to or from a vertex. It also provides methods for
...
public class FlowNetwork {
	 ...
	 * edges. The capacities are integers between 0 and 99 and the flow values
	 * are zero.
	 * 
	 * @param V
	 *            the number of vertices
	...
	/**
	 * Initializes a flow network from an input stream. The format is the number
	 * of vertices <em>V</em>, followed by the number of edges <em>E</em>,
	 * followed by <em>E</em> pairs of vertices and edge capacities, with each
	...
	/**
	 * Returns the edges incident on vertex {@code v} (includes both edges
	 * pointing to and from {@code v}).
	 * 
	 * @param v
	 *            the vertex
	...
	/**
	 * Returns a string representation of the flow network. This method takes
