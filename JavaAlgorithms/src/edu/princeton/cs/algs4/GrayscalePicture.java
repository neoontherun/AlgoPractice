...
/**
 * This class provides methods for manipulating individual pixels of a grayscale
 * image. The original image can be read from a {@code PNG}, {@code GIF}, or
 * {@code JPEG} file or the user can create a blank image of a given dimension.
 ...
 * converted to grayscale if they have different values for the R, G, and B
 * channels. The {@code getGrayscale()} and {@code setGrayscale()} methods use
 * an 8-bit {@code int} to encode the grayscale value, thereby avoiding the need
 * to create temporary {@code Color} objects.
 * <p>
 * A <em>W</em>-by-<en>H</em> picture uses ~ 4 <em>W H</em> bytes of memory,
 * since the color of each pixel is encoded as a 32-bit <code>int</code> (even
 * though, in principle, only ~ <em>W H</em> bytes are needed).
 * <p>
 * For additional documentation, see
 * <a href="https://introcs.cs.princeton.edu/31datatype">Section 3.1</a> of
 * <i>Computer Science: An Interdisciplinary Approach</i> by Robert Sedgewick
 * and Kevin Wayne. See {@link Picture} for a version that supports 32-bit RGB
 * color images.
 *
...
public final class GrayscalePicture implements ActionListener {
	...
	/**
	 * Creates a new grayscale picture that is a deep copy of the argument
	 * picture.
	 *
	 * @param picture
	 *            the picture to copy
	 ...
	 * {@code int} between 0 and 255. Using this method can be more efficient
	 * than {@link #get(int, int)} because it does not create a {@code Color}
	 * object.
	 *
	 * @param col
	 *            the column index
	 ...
	 *            the row index
	 * @param color
	 *            the color (converts to grayscale if color is not a shade of
	 *            gray)
	 * @throws IllegalArgumentException
	 *             unless both {@code 0 <= col < width} and
	 *             {@code 0 <= row < height}
	 ...
	 * grayscale value of a pixel is an integer between 0 and 255.
	 *
