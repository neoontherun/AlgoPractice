 ...
 * <em>global minimum cut</em> in an edge-weighted graph where the edge weights
 * are nonnegative. A <em>cut</em> is a partition of the set of vertices of a
 * graph into two nonempty subsets. An edge that has one endpoint in each subset
 * of a cut is a <em>crossing edge</em>. The weight of a cut is the sum of the
 * weights of its crossing edges. A <em>global minimum cut</em> is a cut for
 * which the weight is not larger than the weight of any other cut.
 * <p>
...
public class GlobalMincut {
	 ...
	 * @throws IllegalArgumentException
	 *             unless vertex {@code v} is between {@code 0} and
	 *             {@code (G.V() - 1)}
	 */
	 ...
	 *            the edge-weighted graph
	 * @param marked
	 *            the array of contracted vertices, where {@code marked[v]} is
	 *            {@code true} if the vertex {@code v} was already contracted;
